...
BEGIN
    DECLARE
    v_id_edificio NUMERIC;
    v_implemento VARCHAR2(70);
    v_cantidad NUMBER;
    v_nombre_servi VARCHAR2(70);
    v_descripcion_servi VARCHAR2(255);
    v_valor_servi NUMBER;
    CURSOR cursor_area_servi IS
        select id_edificio,ae.nombre_implemento,dae.cantidad_implemento,se.nombre,se.descripcion,se.valor from edificio edi
    ...
    OPEN cursor_area_servi;
        DBMS_OUTPUT.put_line('[');
   LOOP 
   FETCH cursor_area_servi into v_id_edificio,v_implemento, v_cantidad, v_nombre_servi,v_descripcion_servi,v_valor_servi; 
      EXIT WHEN cursor_area_servi%notfound; 
      dbms_output.put_line('[{"id":'||v_id_edificio ||',"implemento":"'||v_implemento|| '","cantidadImplemento":' || v_cantidad || ',"servicio":"' || v_nombre_servi  || '","descripcion":"' || v_descripcion_servi || '","valor":' || v_valor_servi || '}],'); 
   END LOOP; 
   DBMS_OUTPUT.put_line(']');
