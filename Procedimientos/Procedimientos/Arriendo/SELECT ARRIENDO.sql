...
BEGIN
   ...
   CURSOR arriendo_cur is 
        ...
        TUR.nombre AS "NOMBRE TOUR",cin.fecha_hora AS "FECHA CHECK IN",cin.validado AS "CHECK IN VALIDADO",cout.fecha_hora AS "HORA CHECK OUT",cout.validado AS "CHECK OUT VALIDADO",
        se.nombre AS "SERVICIO EXTRA",usu.nombre CLIENTE, arr.activo AS ACTIVO 
        FROM ARRIENDO ARR
   ...
   LOOP 
   FETCH arriendo_cur into v_id_arriendo, v_fecha_desde,v_fecha_hasta,v_acompaniantes,v_precio,v_habi_depa,v_nombre_edi,v_tour_name,v_fecha_in,v_in_valida,v_fecha_out,v_out_valida,v_servicio,v_cliente,v_activo; 
      ...
      dbms_output.put_line('{"id":'||v_id_arriendo || ',"arriendoDesde":"'|| v_fecha_desde ||'","arriendoHasta":"'|| v_fecha_hasta ||'","acompaniantes":'||v_acompaniantes||',"precio":'||v_precio||',"departamento":'||v_habi_depa||
                            ...
                            '","nombreServicio":"'||v_servicio||'","cliente":"'||v_cliente||'","activo":'|| v_activo || '},'); 
   END LOOP; 
   DBMS_OUTPUT.put_line(']');
   CLOSE arriendo_cur; 
END; 
